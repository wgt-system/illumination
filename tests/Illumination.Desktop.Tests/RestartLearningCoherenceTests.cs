...
public sealed class RestartLearningCoherenceTests
{
    ...
    public async Task Restart_selected_item_refreshes_real_state_and_a_new_study_can_select_it()
    {
        ...
        // Use the real Study path to move the item out of the immediate queue first.
        ...
        var future = Assert.Single(vm.LearningItems, x => x.Id == item.Id);
        ...
        var restarted = Assert.Single(vm.LearningItems, x => x.Id == item.Id);
        Assert.True(restarted.IsNew);
        Assert.Equal(Now, restarted.DueAt);
        Assert.Contains("restarted", vm.StatusMessage, StringComparison.OrdinalIgnoreCase);
